...
# Global formula from file 5 clim phase: log_size_t0 * (year + site + aspect.cat + clim_vars)
...
# Top 10 comparison — separate file per vital rate
write_top10 <- function(d0, d1, vr_name) {
  t0 <- as.data.frame(d0) %>% slice_head(n = 10) %>% mutate(nAGQ = 0, rank = row_number())
  t1 <- as.data.frame(d1) %>% slice_head(n = 10) %>% mutate(nAGQ = 1, rank = row_number())
  out <- bind_rows(t0, t1) %>% select(nAGQ, rank, df, logLik, AICc, delta, weight, everything())
  write_csv(out, paste0("r/modelFitResults/nAGQ_validation_top10_", vr_name, ".csv"))
}

write_top10(d_surv_0, d_surv_1,  "survival")
write_top10(d_repro_0, d_repro_1, "repro")
write_top10(d_nflrs_0, d_nflrs_1, "nflrs")
write_csv(as.data.frame(d_growth) %>% slice_head(n = 10) %>% mutate(rank = row_number()),
          "r/modelFitResults/nAGQ_validation_top10_growth.csv")
